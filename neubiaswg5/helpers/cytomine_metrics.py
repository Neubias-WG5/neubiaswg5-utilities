from cytomine.models import Model, Collection, ImageInstance, ImageGroup, ImageInstanceCollection, ImageGroupCollection
...
class ImageGroupMetricResult(Model):
    def __init__(self, id_metric=None, id_job=None, id_image=None, value=None, **attributes):
        super(ImageGroupMetricResult, self).__init__()
        self.metric = id_metric
        self.job = id_job
        self.image = id_image
        self.value = value
...
class ImageGroupMetricResultCollection(Collection):
    def __init__(self, filters=None, max=0, offset=0, **parameters):
        super(ImageGroupMetricResultCollection, self).__init__(ImageGroupMetricResult, filters, max, offset)
        self._allowed_filters = [None]
        self.set_parameters(parameters)
...
def get_metric_result_collection(image_or_collection):
    ...
    elif _check_type(image_or_collection, ImageGroup, ImageGroupCollection):
        return ImageGroupMetricResultCollection()
    else:
...
def get_metric_result(image, id_metric=None, id_job=None, value=None, **attributes):
    ...
    elif _check_type(image, ImageGroup, ImageGroupCollection):
        return ImageGroupMetricResult(id_metric=id_metric, id_job=id_job, id_image=image.id, value=value, **attributes)
    else:
